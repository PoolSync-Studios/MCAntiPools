    ...
    static void FixImports(byte[] pe, IntPtr baseAddr, int importDirRva)
    {
        ...
        while (true)
        {
            ...
            while (true)
            {
                ...
                if (val == 0) break;
                // Resolve by name only (skip ordinal imports – this EXE doesn't need them)
                if ((val & 0x80000000L) != 0 || (val & 0x8000000000000000L) != 0)
                {
                    // Ordinal import – skip (rarely used, not needed for BraveInstaller)
                }
                else
                {
                    int nameOff = RvaToOffset(pe, (int)(val & 0x7FFFFFFF));
                    string fn = Marshal.PtrToStringAnsi(IntPtr.Add(baseAddr, nameOff + 2));
                    IntPtr func = GetProcAddress(hMod, fn);
                    if (Environment.Is64BitProcess)
                        Marshal.WriteInt64(IntPtr.Add(baseAddr, iatOff), (long)func);
                    else
                        Marshal.WriteInt32(IntPtr.Add(baseAddr, iatOff), (int)func);
                }
